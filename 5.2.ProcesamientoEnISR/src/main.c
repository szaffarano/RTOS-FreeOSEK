...
static unsigned int enlapsed_time;
...
TASK(taskLED) {
	...
	while (1) {
		if (queue_pop(&queue, &value, 10) == QUEUE_OK) {
			duty = value < 1000 ? value : 990;
		}

		Chip_GPIO_SetPinState(LPC_GPIO, 2, 0, true);

		SetRelAlarm(WakeUpLED, duty, 0);
		WaitEvent(EventLED);
		ClearEvent(EventLED);

		Chip_GPIO_SetPinState(LPC_GPIO, 2, 0, false);
		SetRelAlarm(WakeUpLED, 1000 - duty, 0);
		WaitEvent(EventLED);
		ClearEvent(EventLED);
	}
}
...
TASK(TareaFalling) {
	...
	if (!Chip_GPIO_GetPinState(LPC_GPIO, 0, 15)) {
		...
		// comienzo a contar el tiempo en que el botón está presionado
		enlapsed_time = ticks;
	} else {
		Chip_GPIOINT_SetIntFalling(LPC_GPIOINT, GPIOINT_PORT0, (1 << 15));
	}
...
TASK(TareaRising) {
	...
	if (Chip_GPIO_GetPinState(LPC_GPIO, 0, 15)) {
		...
		// calculo el tiempo que estuvo presionado
		enlapsed_time = ticks - enlapsed_time;

		// informo a la tarea el tiempo
		queue_push(&queue, enlapsed_time, 100);
	} else {
		Chip_GPIOINT_SetIntRising(LPC_GPIOINT, GPIOINT_PORT0, (1 << 15));
	}
